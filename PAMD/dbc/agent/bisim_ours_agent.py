...
class BisimOursAgent(object):
    """Bisimulation metric algorithm with our distance."""
    def __init__(
        self,
        obs_shape,
        action_shape,
    ...
    def update_encoder(self, obs, action, reward, L, step):
        ...
        perm = torch.randperm(batch_size, device=h.device)
        h2 = h[perm]
        ...
        B,d=h.shape
        G = self.metric_net(h,h2)
        tr=torch.diagonal(G,dim1=1,dim2=2).sum(dim=1)
        
        G=G/(tr.view(B,1,1)+eps)
        delta = (h - h2).unsqueeze(-1)
        z_dist_squared = torch.bmm(
        torch.bmm(delta.transpose(1, 2), G), 
        delta
        ).squeeze(-1)
        z_dist = torch.sqrt(torch.clamp(z_dist_squared, min=1e-8))
        
        r_dist = F.smooth_l1_loss(reward, reward2, reduction='none')
        
        if self.transition_model_type == '':
            
            mu1, mu2 = pred_next_latent_mu1, pred_next_latent_mu2
        ...
        I=torch.eye(d,device=G.device).unsqueeze(0)
        auxloss1=((G-I/d).pow(2).mean())
        loss=loss-1e-3*auxloss1
        
        L.log('train/DBC_loss', loss.item(), step)
        ...
        L.log('train/transition_dist', transition_dist.mean().item(), step)
        L.log('train/withdetach', 0, step)

        # ---------- trace (pre-normalization) ----------
        L.log('train/trace_G', tr.mean().item(), step)
        L.log('train/aux_loss1', auxloss1.item(), step)
        return loss
    ...
    def save(self, model_dir, step):
        ...
        torch.save(
            self.metric_net.state_dict(),
            '%s/metric_net_%s.pt' % (model_dir, step)
        )
