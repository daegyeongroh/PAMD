...
import utils
from encoder_mico import make_encoder
...
class SimSR(nn.Module):
    ...
    def encode(self, x, detach=False, ema=False):
        ...
        if detach:
            z_out = z_out.detach()
        return z_out
def mico_U_pair(z, z2, beta=0.1, eps=1e-8):
    n1 = (z ** 2).sum(dim=1)
    n2 = (z2 ** 2).sum(dim=1)
    norm_term = 0.5 * (n1 + n2)

    dot = (z * z2).sum(dim=1)
    denom = (n1.sqrt() + eps) * (n2.sqrt() + eps)
    cos = (dot / denom).clamp(-1 + 1e-6, 1 - 1e-6)
    sin = torch.sqrt(torch.clamp(1.0 - cos * cos, min=0.0) + eps)
    theta = torch.atan2(sin, cos)
    return norm_term + beta * theta

class micoSacAgent(object):
    """MICo with SAC."""
    ...
    def __init__(
        ...
        self.mico_alpha = 1e-5 
        self.mico_beta = 0.1
        self.mico_target_update_freq = 8000  

        # MICo uses its OWN target encoder (hard sync), not the SAC critic_target encoder (polyak)
        self.mico_encoder_target = copy.deepcopy(self.critic.encoder).to(self.device)
        for p in self.mico_encoder_target.parameters():
            p.requires_grad = False
        self.mico_encoder_target.eval()
        self.train()
    ...
    def update_critic(self, obs, action, reward, next_obs, not_done, L, step):
        ...
        if self.encoder_type == 'pixel' and self.mico_alpha > 0.0:
            B = obs.size(0)
            z = self.critic.encoder(obs)
            perm = torch.randperm(B, device=z.device)

            with torch.no_grad():
                z_y = self.mico_encoder_target(obs[perm])
                z_next = self.mico_encoder_target(next_obs)
                z_next2 = z_next[perm] 

            z2 = z[perm]
            z_next2 = z_next[perm]
            reward2 = reward[perm]
            z_diff = mico_U_pair(z, z_y, beta=self.mico_beta)  
            next_diff = mico_U_pair(z_next, z_next2, beta=self.mico_beta)
            r_diff = torch.abs(reward - reward2).squeeze(-1)

            bisimilarity = r_diff + self.discount * next_diff

            mico_loss = F.smooth_l1_loss(z_diff, bisimilarity.detach())
        else:
            mico_loss = 0.0
        total_loss = (1.0 - self.mico_alpha) * critic_loss + self.mico_alpha * mico_loss
        if step % self.log_interval == 0:
            L.log('train_critic/critic_loss', critic_loss, step)
            L.log('train_critic/mico_loss', mico_loss, step)
    ...
    def update(self, replay_buffer, L, step):
        ...
        # TD + MICo is handled inside update_critic (single backward)
        ...
        # SAC target updates (polyak) as-is
        ...
        # MICo target encoder update (HARD sync, infrequent)
        if step % self.mico_target_update_freq == 0:
            self.mico_encoder_target.load_state_dict(self.critic.encoder.state_dict())
            self.mico_encoder_target.eval()
