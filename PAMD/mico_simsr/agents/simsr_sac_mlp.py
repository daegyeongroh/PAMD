...
class SimSRSacmlpAgent(object):
    ...
    def __init__(
        ...
        if self.encoder_type == 'pixel':
            ...
            self.distance = DistanceMLP(encoder_feature_dim).to(device)
    ...
    def update_encoder(self, obs, next_obs, reward, action, L, step):
        ...
        z_dist = self.distance(z,z2).squeeze(-1)
        next_dist = self.distance(pred_next,pred_next2).squeeze(-1)
        # --- bisim target ---
    ...
    def save(self, model_dir, step):
        ...
        torch.save(
            self.distance.state_dict(), '%s/distance_%s.pt' % (model_dir, step)
        )
