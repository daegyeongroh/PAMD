...
def rehu(x, delta=1.0, eps=1e-12):
    return torch.where(
        x <= 0,
        torch.zeros_like(x),
        torch.where(
...
class MetricNet(nn.Module):
    ...
    def forward(self, z1, z2):
        ...
        def build_G(z_a, z_b):
            z_concat = torch.cat([z_a, z_b], dim=1)
            L_vec = self.net(z_concat)

            L = torch.zeros(B, self.in_dim, self.in_dim, device=device)
            tril_idx = torch.tril_indices(self.in_dim, self.in_dim, offset=0)
...
class MICoSimSRourAgent(object):
    """MICo/SimSR with our distance."""
    ...
    def __init__(
        ...
        if self.encoder_type == 'pixel':
            ...
            self.metric_net = MetricNet(encoder_feature_dim).to(device)
            ...
            self.encoder_optimizer = torch.optim.Adam(
                ...
                list(self.metric_net.parameters()), 
                lr=encoder_lr
            )
        self.train()
    ...
    def update_encoder(self, obs, next_obs, reward, action, L, step):
        ...
        # --- encode ---
        z = self.SimSR.encode(obs) 

        with torch.no_grad():
            z_tar = self.SimSR.encode(obs, ema=True)
            pred_next = self.transition_model.sample_prediction(
        ...
        z2 = z[perm]
        pred_next2 = pred_next[perm]
        reward2 = reward[perm]

        # --- build metrics ---
        G = self.metric_net(z,z2)
        G_next = self.metric_net(pred_next,pred_next2)

        # --- TRACE NORMALIZATION  ---
        tr = torch.diagonal(G, dim1=1, dim2=2).sum(dim=1)    
        tr_next = torch.diagonal(G_next, dim1=1, dim2=2).sum(dim=1)
        G_ori=G
        G = G / (tr.view(B, 1, 1) + eps)
        G_next = G_next / (tr_next.view(B, 1, 1).detach() + eps)

        # --- PAMD distances: sqrt( (x-y)^T G (x-y) ) ---
        dz = (z - z2).unsqueeze(2)             
        dp = (pred_next - pred_next2).unsqueeze(2)

        z_dist2 = torch.bmm(dz.transpose(1, 2), torch.bmm(G, dz)).squeeze(-1).squeeze(-1)         
        next_dist2 = torch.bmm(dp.transpose(1, 2), torch.bmm(G_next, dp)).squeeze(-1).squeeze(-1) 

        z_dist = torch.sqrt(z_dist2 + eps)        
        next_dist = torch.sqrt(next_dist2 + eps)    

        # --- bisim target ---
        r_dist = torch.abs(reward - reward2).squeeze(-1)  
        bisim_target = r_dist + self.discount * next_dist 

        # --- main loss: match distances ---
        loss = F.smooth_l1_loss(z_dist, bisim_target.detach())
        I = torch.eye(d, device=z.device).unsqueeze(0)
        auxloss=((G - I/d).pow(2).mean())
        loss = loss - 1e-3 * auxloss


        if step % self.log_interval == 0:
            
            L.log('train/G_isfinite_frac', torch.isfinite(G).float().mean().item(), step)
            L.log('train/G_has_nan', float(torch.isnan(G).any().item()), step)
            L.log('train/G_has_inf', float(torch.isinf(G).any().item()), step)
            L.log('train/SimSR_loss', loss, step)
            L.log('train/SimSR_aux_loss', auxloss, step)
            L.log('train/bisimilarity', bisim_target.mean().item(), step)
            L.log('train/z_dist', z_dist.mean().item(), step)
            L.log('train/r_dist', r_dist.mean().item(), step)
            sign, logabsdet = torch.linalg.slogdet(G + eps * torch.eye(d, device=z.device).unsqueeze(0))
            L.log('train/logabsdet_G', logabsdet.mean().item(), step)
            L.log('train/slogdet_sign_frac_pos', (sign > 0).float().mean().item(), step)

            trace_G = torch.diagonal(G_ori, dim1=1, dim2=2).sum(dim=1)
            L.log('train/trace_G', trace_G.mean().item(), step)
    ...
    def save(self, model_dir, step):
        ...
        torch.save(
            self.metric_net.state_dict(), '%s/metric_net_%s.pt' % (model_dir, step)
        )
        

    def save_SimSR(self, model_dir, step):
        torch.save(
            self.critic.encoder.state_dict(), '%s/SimSR_%s.pt' % (model_dir, step)
        )
